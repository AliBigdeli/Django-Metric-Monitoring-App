...
from .serializers import DeviceSerializer, MetricSerializer
...
class MetricListView(APIView):
    ...
    def get(self, request, token, *args, **kwargs):
        ...
        coordinates = Metric.objects.filter(device=device.id)[:20]
        serializer = MetricSerializer(coordinates, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class MetricCurrentView(APIView):
    # add permission to check if user is authenticated
    permission_classes = [permissions.IsAuthenticated]
    ...
    def get(self, request, token, *args, **kwargs):
        ...
        coordinate = Metric.objects.filter(
            device=device_instance.id).latest('created_date')
        serializer = MetricSerializer(Metric, many=False)
        return Response(serializer.data, status=status.HTTP_200_OK)
